...
适用于拿到 release zip 后的本地使用者。不需要手动输入 `npm install` / `npm start`。

## 启动步骤

1. 解压 `Content-Engine-Lite-v0.2.0.zip` 到一个固定文件夹。
2. 双击根目录里的 `start-windows.bat`。
3. 首次运行如果没有 `node_modules/`，脚本会自动执行 `npm install`，需要联网，可能需要几分钟。
4. 服务启动后，浏览器会自动打开 `http://127.0.0.1:3000`。

启动窗口会保持打开，用来显示运行日志。关闭这个窗口通常会停止本地服务。
...
如果提示没有检测到 Node.js 或 npm，请先安装 Node.js LTS：
...
通常是网络、代理、权限或 npm 源问题。可以确认电脑能访问 npm registry，或把项目解压到权限限制较少的目录，例如：

```text
D:\Content-Engine-Lite\
```
